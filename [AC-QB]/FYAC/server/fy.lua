...
AddEventHandler("entityCreating", function(id)
	...
	AddEventHandler("entityCreating", function(entity)
	 ...
	 AddEventHandler(value.trigger, function(source,args)
	 local xPlayer = QBCore.Functions.GetPlayer(source)
	 if args ~= nil then
	 if value.value == -1 and args == -1 then
	 CancelEvent()
	 sendToDiscord(FYAC_S.Log["Trigger Inputs"],source,FYAC_L.locales[FYAC_L.language].triggerserver,"Event:\n"..value.trigger.."\n\n**Reason:**"..FYAC_L.locales[FYAC_L.language].cashlimit.."\n\n**Info:**Limit exceeded:"..value.value.."\n\n",10038562)
	 Citizen.Wait(1000)
	 banned(source,FYAC_L.locales[FYAC_L.language].triggerserver)
	 elseif value.value == -1 then
	 elseif not (value.value <= tonumber(args)) then
	 else
	 CancelEvent()
	 sendToDiscord(FYAC_S.Log["Trigger Inputs"],source,FYAC_L.locales[FYAC_L.language].triggerserver,"Event:\n"..value.trigger.."\n\n**Reason:**"..FYAC_L.locales[FYAC_L.language].cashlimit.."\n\n**Info:**Limit exceeded:"..value.value.."\n\n\n",10038562)
	 Citizen.Wait(1000)
	 banned(source,"The amount of money printed on it by cheating.")
	 end
	 end
	 end)
	 end
